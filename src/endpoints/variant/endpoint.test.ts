...
describe("VariantEndpoint", () => {
  ...
  describe("list", () => {
    ...
    it("should send a GET request to root with search parameters", async () => {
      ...
      await endpoint.list({
        filter: { name: "123" },
        expand: { product: { owner: true } },
      });

      expect(fetchSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          `${ENDPOINT_URL}?limit=100&expand=product.owner&filter=name%3D123`,
        ),
        expect.objectContaining({ method: "GET" }),
      );
    });
  ...
  describe("get", () => {
    ...
    it("should send a GET request with search parameters", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          `${ENDPOINT_URL}/123?limit=100&expand=product.owner`,
        ),
        expect.objectContaining({ method: "GET" }),
      );
    });
  ...
  describe("first", () => {
    ...
    it("should send a GET request with search parameters", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        expect.stringContaining(`${ENDPOINT_URL}?limit=1&expand=product.owner`),
        expect.objectContaining({ method: "GET" }),
      );
    });
  });
  ...
  describe("create", () => {
    ...
    it("should send a POST request with search parameters", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          `${ENDPOINT_URL}?limit=100&expand=product.owner`,
        ),
        expect.objectContaining({
          method: "POST",
          body: JSON.stringify({}),
